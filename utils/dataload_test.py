...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, crop_size, hazy_flist, clean_flist=None, clean_path=None, hazy_path=None, transmission_flist=None, augment=True, split='unpair'):

        self.clean_data = self.load_flist(clean_flist)   # list
        self.noisy_data = self.load_flist(hazy_flist)   # list
            ...
            if self.split in ['hazy', 'hazy_various']:
                ...
                while(True):
                    ...
                    if np.array(img_clean).shape is None:
                        ...
                        print(self.clean_data[clean_index])
                        print(self.clean_data[clean_index])
                    if min(np.array(img_clean).shape[0:2]) >= self.input_size:
                        break
                ...
                while(True):
                    ...
                    if min(np.array(img_noisy).shape[0:2]) >= self.input_size:
                        break

                img_noisy = self.convert_to_rgb(img_noisy)
                img_clean = self.convert_to_rgb(img_clean)
                # import cv2
                # img_clean = cv2.imread(self.clean_data[clean_index])
    ...
    def get_gt_path(self, path):
        ...
        if self.split == 'pair_train':
            ...
            # prefix = str.split(filename, '_')[0]
            prefix = str.split(filename, '.')[0]
            # print(prefix, str.split(filename, '.')[1])
            # print(prefix)
            # gt_path = os.path.join(self.config.TEST_CLEAN_PATH, prefix+'.png')
            # gt_path = os.path.join(self.config.TEST_CLEAN_PATH, prefix + '.jpg')
            # gt_path = os.path.join(dirname, prefix + filename[-4:])
            gt_path = os.path.join(dirname, filename)
