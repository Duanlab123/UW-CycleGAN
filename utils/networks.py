...
class HazeRemovalNet(BaseNetwork):
    ...
    def forward(self, x_0, A,require_paras=False, use_guided_filter=False):
        ...
        beta=beta.clamp(self.MIN_BETA_b,self.MAX_BETA_b)
        #beta[:, 0, :, :] = self.MIN_BETA_r + (self.MAX_BETA_r-self.MIN_BETA_r)*(torch.tanh(beta[:, 0, :, :]) + 1) / 2
        #beta[:, 1, :, :] = self.MIN_BETA_g + (self.MAX_BETA_g-self.MIN_BETA_g)*(torch.tanh(beta[:, 1, :, :]) + 1) / 2
        #beta[:, 2, :, :] = self.MIN_BETA_b + (self.MAX_BETA_b-self.MIN_BETA_b)*(torch.tanh(beta[:, 2, :, :]) + 1) / 2
        ...
        if require_paras:
            return ((x_0-A)/t + A).clamp(0, 1), d, beta, t
        else:
...
class HazeProduceNet(BaseNetwork):
    ...
    def forward_random_parameters(self, image, depth,water_type=1,water_depth=0,water_illum=[1,0]):  # x:NCHW, ex:e^(-d(x)), N,1,H,W, beta:N,1,1,1
        ...
        x, y = np.meshgrid(np.linspace(0, depth.shape[2] - 1, depth.shape[2]),
        ...
        L_t3, v = get_art_light(x, y, eta, depth, sigma, x_c=np.random.randint(0, x.shape[1]),
                                y_c=np.random.randint(0, x.shape[0]), L_art=np.random.uniform(0.6, 0.99, 1), Z_l=z_l,
                                r_l=r_l)
        L_t = water_illum[0] * L_t1 + water_illum[1] * L_t2 + 0 * L_t3  # L_t3
        direct = np.multiply(np.multiply(image, L_t), t)
        #---------------------------------------------------------------
        t_haze = np.exp(np.multiply(-1, np.multiply(depth, eta)))
        A = np.random.uniform(0.6, 0.99, 1) * L_t
        image_haze = np.multiply(A, 1 - t_haze)
        B = A
        I = direct + image_haze
...
class TransmissionEstimator(nn.Module):
    ...
    def get_dark_channel(self, x):  # x --> [2, 3, 256, 256]
        # x = torch.min(x[:, 1:3, :, :], dim=1, keepdim=True)[0]  # dim=1 求每个像素[G, B]的最小值

        x = torch.min(x, dim=1, keepdim=True)[0]   # dim=1 求每个像素RGB三元组的最小值
