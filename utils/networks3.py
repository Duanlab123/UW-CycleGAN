...
class TransNet(BaseNetwork):
    def __init__(self, base_channel_nums, init_weights=False, use_pretrained=False, use_dc_A=False):
...
class BetaNet(BaseNetwork):
    ...
    def forward(self, x_0, t, require_paras=False, use_guided_filter=False):
...
class HazeProduceNet(BaseNetwork):
    ...
    def forward_random_parameters(self, image, depth, water_type=1, water_depth=0, water_illum=[1, 0]):  # x:NCHW, ex:e^(-d(x)), N,1,H,W, beta:N,1,1,1
        ...
        water_depth = np.random.uniform(0.4, 5, 1)
