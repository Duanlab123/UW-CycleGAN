...
class Dataset(torch.utils.data.Dataset):
    ...
    def __len__(self):
        return len(self.under_data) #if len(self.clean_data)>len(self.under_data) else len(self.under_data)
    ...
    def load_item(self, index):
        ...
        under = cv2.imread(self.under_data[index])
        under_refer = cv2.imread(self.under_refer[index])
        ...
        # else:
        #     under_refer = under
        # under = self.transform(Image.open(under[index]).convert('RGB'))
        # under = Variable(under).type(Tensor).unsqueeze(0)
        # under = self.transform(Image.open(under[index]).convert('RGB'))
        # under = Variable(under).type(Tensor).unsqueeze(0)
        # under_refer = transform(Image.open(under_refer[index]).convert('RGB'))
        # under_refer = Variable(ground).type(Tensor).unsqueeze(0)
